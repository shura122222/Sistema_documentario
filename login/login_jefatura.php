...
function mostrarNotificacionUsuarioIncorrecto($usuario = '', $mensaje = '') {
    echo "<!DOCTYPE html>";
    echo "<html lang='es'>";
    echo "<head>";
    echo "<meta charset='UTF-8'>";
    echo "<meta name='viewport' content='width=device-width, initial-scale=1.0'>";
    echo "<title>Usuario No Encontrado - Jefatura</title>";
    echo "<link href='https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css' rel='stylesheet'>";
    echo "<style>";
    echo "body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: linear-gradient(135deg, #0f2027 0%, #203a43 20%, #2c5530 100%); margin: 0; padding: 0; min-height: 100vh; display: flex; align-items: center; justify-content: center; }";
    ...
    echo ".btn-primary { background: #3b82f6; color: white; }";
    echo ".btn-primary:hover { background: #2563eb; transform: translateY(-1px); }";
    ...
    echo $mensaje ?: 'El usuario ingresado no está registrado en el área de Jefatura.';
    echo "</div>";
    ...
    echo "<i class='fas fa-shield-alt'></i> Por seguridad, solo usuarios autorizados pueden acceder al sistema de Jefatura.";
    echo "</div>";
...
function mostrarFormularioReintento($usuario, $nombreCompleto, $cargo, $error = '') {
    echo "<!DOCTYPE html>";
    echo "<html lang='es'>";
    echo "<head>";
    echo "<meta charset='UTF-8'>";
    echo "<meta name='viewport' content='width=device-width, initial-scale=1.0'>";
    echo "<title>Verificación de Contraseña - Jefatura</title>";
    echo "<link href='https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css' rel='stylesheet'>";
    echo "<style>";
    echo "body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: linear-gradient(135deg, #0f2027 0%, #203a43 20%, #2c5530 100%); margin: 0; padding: 0; min-height: 100vh; display: flex; align-items: center; justify-content: center; }";
    ...
    echo ".form-group input:focus { outline: none; border-color: #3b82f6; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1); }";
    echo ".btn-group { display: flex; gap: 1rem; margin-top: 2rem; }";
    echo ".btn { flex: 1; padding: 0.75rem 1.5rem; border: none; border-radius: 8px; font-size: 1rem; font-weight: 600; cursor: pointer; transition: all 0.3s; text-decoration: none; text-align: center; display: inline-block; }";
    echo ".btn-primary { background: #3b82f6; color: white; }";
    echo ".btn-primary:hover { background: #2563eb; transform: translateY(-1px); }";
    ...
    echo "<p><strong>Área:</strong> Jefatura</p>";
    echo "</div>";
    
    echo "<form method='POST' action=''>";
    echo "<input type='hidden' name='usuario' value='" . htmlspecialchars($usuario) . "'>";
    echo "<input type='hidden' name='area' value='jefatura'>";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($inputUsuario) || empty($inputPassword)) {
        if ($isRetry) {
            ...
            $pdo = conectarDB();
            if ($pdo) {
                $stmt = $pdo->prepare("SELECT usuario, nombre_completo, cargo FROM usuarios_admin WHERE usuario = ? AND area = 'jefatura' LIMIT 1");
                $stmt->execute([$inputUsuario]);
                $user = $stmt->fetch();
                if ($user) {
                    mostrarFormularioReintento($user['usuario'], $user['nombre_completo'], $user['cargo'], 'La contraseña es requerida');
                    exit;
                }
            }
        }
    ...
    // Validar que sea para jefatura
    if ($inputArea !== 'jefatura') {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            ...
            FROM usuarios_admin 
            WHERE usuario = ? AND area = 'jefatura'
            LIMIT 1
        ");
        ...
        if ($user) {
            ...
            if ($passwordValida) {
                ...
                echo "<meta charset='UTF-8'>";
                echo "<meta name='viewport' content='width=device-width, initial-scale=1.0'>";
                echo "<title>Acceso Autorizado - Jefatura</title>";
                echo "<link href='https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css' rel='stylesheet'>";
                echo "<style>";
                echo "body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: linear-gradient(135deg, #0f2027 0%, #203a43 20%, #2c5530 100%); margin: 0; padding: 0; min-height: 100vh; display: flex; align-items: center; justify-content: center; }";
                ...
                echo "<p class='success-message'>Bienvenido al Sistema de Jefatura</p>";
                ...
                echo "<i class='fas fa-arrow-right'></i> Redirigiendo al Panel de Jefatura...";
                ...
                echo "    window.location.href = '../administracion/jefatura.php';";
                ...
                mostrarFormularioReintento($user['usuario'], $user['nombre_completo'], $user['cargo'], $errorMessage);
                exit;
            }
            
        } else {
            // USUARIO NO ENCONTRADO
            mostrarNotificacionUsuarioIncorrecto($inputUsuario, 'El usuario "' . htmlspecialchars($inputUsuario) . '" no está registrado en el área de Jefatura. Por favor, ingrese un usuario válido.');
            exit;
        }
